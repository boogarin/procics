...
import { ChartColumnBig, CircleFadingArrowUp, GlobeLock, LayoutDashboardIcon, TabletSmartphone, Wrench } from "lucide-react";
import { FaLinkedin } from "react-icons/fa";
import { TypingTextRotator } from "./components/TypingText";
...
export default function Home() {
  ...
  return (
    ...
    className="flex p-6 md:p-10 lg:p-14 justify-between w-[calc(100%-5px)] lg:w-[calc(100%-40px)] xl:w-[calc(100%-80px)] 2xl:w-[calc(100%-600px)] mx-auto">
      ...
      <div className='flex space-x-10 items-center'>
        <ul className='hidden md:flex space-x-10 text-white font-light'>
          ...
          <li onClick={() => {projects.current?.scrollIntoView({behavior: "smooth"})}} className="font-jet hover:underline cursor-pointer z-10 transition ease-in-out duration-100 hover:font-extrabold hover:scale-105">PROJETOS</li>
        </ul>
      </div>
    </motion.div>
    ...
    {/* SECTION ONE */}
    <motion.div initial={{opacity: 0}} whileInView={{opacity: 1, transition: { duration: 1}}} viewport={{once: true}}
    className="flex p-10 -mb-5 md:-mb-0 lg:p-12 h-80 md:h-72 justify-center lg:justify-normal w-[calc(100%-5px)] lg:w-[calc(100%-40px)] xl:w-[calc(100%-80px)] 2xl:w-[calc(100%-600px)] mx-auto">
      
      <div className="flex flex-col -mt-10">
        <div className="flex flex-col z-10">
          <div className="flex space-x-5 lg:justify-normal">
      ...
      <motion.div initial={{opacity: 0}} whileInView={{opacity: 1, transition: {duration: 5, delay: 1}}} viewport={{once: true}} className="absolute mt-20 md:mt-0 lg:top-0 z-0">
    ...
    className="bg-white bg-gradient-to-b from-yellow-100/50 via-pink-100/50 to-purple-200  flex w-full py-5 pt-20 md:pt-8 lg:pt-6 xl:pt-2">
      <div className="w-[calc(100%-5px)] lg:w-[calc(100%-40px)] xl:w-[calc(100%-80px)] 2xl:w-[calc(100%-600px)] mx-auto p-6 md:p-10 lg:p-14 text-[#4d4d4d] z-20">
        ...
        <motion.div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 md:flex-row gap-5 items-stretch">
          <motion.div
          ...
          <motion.div
            ...
            custom={3}
          >
            <Service title="Desenvolvimento de Apps e Sites" subtitle="Aplicativos voltados para o público em geral ou interno da empresa..." Icon={TabletSmartphone} />
          </motion.div>
          <motion.div
            className="flex-1 h-full"
            initial="hidden"
            ...
            custom={4}
          >
            <Service title="Gestão de Tráfegos" subtitle="Aumentar o alcance e obter resultados efetivos..." Icon={ChartColumnBig} />
          </motion.div>
          <motion.div
            className="flex-1 h-full"
            initial="hidden"
      ...
      <motion.div className="flex flex-col md:flex-row start justify-between items-center md:items-start w-[calc(100%-5px)] lg:w-[calc(100%-40px)] 2xl:w-[calc(100%-600px)] mx-auto p-6 md:p-10 lg:p-14">
        <div className="text-center mb-10 -mt-5 md:mb-0 md:text-left md:w-1/3 flex flex-col">
          ...
          <motion.div className="flex flex-col" initial={{opacity: 0, y: -10}} whileInView={{opacity: 1, y: 0, transition: {duration: 2, delay: 2.8}}} viewport={{once: true}}>
            ...
            <Link href="https://wa.me/5511983199825" target="_blank"
            className="flex space-x-3 items-center justify-center md:justify-normal text-emerald-300 hover:text-emerald-200 duration-300">
                <SiWhatsapp size={20}/>
    ...
    className='w-full items-center justify-between flex flex-row bg-black md:h-[220px]'>
      <div className="flex flex-col md:grid md:grid-cols-2 w-[calc(100%-5px)] lg:w-[calc(100%-40px)] 2xl:w-[calc(100%-600px)] mx-auto text-white p-14">
